...
function formatAuraSomaLabel(filename) {
  const match = filename.match(/Equilibrium_(\d+)/);
  if (match) return `Equilibrium ${match[1]}`;
  return filename.replace(".jpg", "");
}

function renderAuraSomaGallery() {
  const container = document.getElementById("aura-soma-gallery");
  if (!container) return;
  container.innerHTML = "";
  auraSomaImages.forEach((file) => {
    const label = formatAuraSomaLabel(file);
    const src = `${auraSomaBasePath}/${file}`;
