...
const equilibriumBasePath = "images/aura-soma/equilibrium/";
...
function renderEquilibriumGallery() {
  ...
  container.innerHTML = "";
  equilibriumImages.forEach((file, index) => {
    const match = file.match(/Equilibrium_(\d+)/);
    const number = match ? match[1] : index + 1;
    const label = `Equilibrium ${number}`;
    const card = document.createElement("a");
    card.className = "gallery-item card";
    card.href = `${equilibriumBasePath}${file}`;
    card.target = "_blank";
    card.rel = "noopener";
    card.innerHTML = `
      <img src="${equilibriumBasePath}${file}" alt="${label}" loading="lazy" />
      <div class="gallery-meta">
        <span class="gallery-title">${label}</span>
        <span class="gallery-link">查看原圖 ↗</span>
      </div>
    `;
    container.appendChild(card);
  });

  const countEl = document.getElementById("equilibrium-count");
  if (countEl) {
    countEl.textContent = equilibriumImages.length;
  }
}
